...
then
    ...
    export MY_PUBLIC_IP=$(ip -o addr list | grep -v virbr | grep -v docker | grep -v tun | grep -v 127.0.0.1 | grep -v $MY_PRIVATE_IP | grep -v inet6 | grep inet | awk '{ print $2,$3,$4 }' | awk '{ print $3 }' | cut -d '/' -f 1 | head -n 1)
