...
then
	syslog_netcat "Usage: cb_filebench.sh <load level> <load duration> <load_id>"
	exit 1
fi
...
report_app_metrics load_id:${LOAD_ID}:seqnum load_level:${LOAD_LEVEL}:load load_duration:${LOAD_DURATION}:sec latency:$lat:msec throughput:$tp:tps bandwidth:$bw:MBps
