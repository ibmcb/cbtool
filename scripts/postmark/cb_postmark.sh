...
then
    syslog_netcat "Usage: cb_postmark.sh <load_profile> <load level> <load duration> <load_id> [sla_targets]"
    exit 1
fi
...
syslog_netcat "Benchmarking postmark SUT: POSTMARK=${my_ip_addr} with LOAD_LEVEL=${LOAD_LEVEL} and LOAD_DURATION=${LOAD_DURATION} (LOAD_ID=${LOAD_ID} and LOAD_PROFILE=${LOAD_PROFILE})"

OUTPUT_FILE=$(mktemp)

execute_load_generator "${CMDLINE}" ${OUTPUT_FILE} ${LOAD_DURATION}

tp=$(cat $OUTPUT_FILE | grep "of transactions" | cut -d '(' -f 2 | awk '{ print $1 }')
#cfps=$(cat $OUTPUT_FILE | grep created | cut -d '(' -f 2 | awk '{ print $1 }') 
#rfps=$(cat $OUTPUT_FILE | grep read | grep -v bytes | cut -d '(' -f 2 | awk '{ print $1 }')
#afps=$(cat $OUTPUT_FILE | grep appended | cut -d '(' -f 2 | awk '{ print $1 }')
#dfps=$(cat $OUTPUT_FILE | grep deleted | cut -d '(' -f 2 | awk '{ print $1 }')
rbw=$(cat $OUTPUT_FILE | grep read | grep bytes | cut -d '(' -f 2 | awk '{ print $1 }')
wbw=$(cat $OUTPUT_FILE | grep read | grep bytes | cut -d '(' -f 2 | awk '{ print $1 }') 

bw=$(echo "scale=2; ($rbw + $wbw)/1" | bc -l)

syslog_netcat "postmark run complete. Will collect and report the results $tp $lat"
