...
(bash -c "sleep 5; su ${who} -c 'if [ x$(cb cldshow vpn | grep server_discovery | cut -d \"|\" -f 4) == xTrue ] ; then ${dir}/../../cb cldalter vpn server_bootstrap $VPNIP' >> $logpath ; else echo modifying bootstrap to $VPNIP not allowed >> $logpath; fi" &)
