	...
	for (vapp_type in vapp_types) {
		...
		for (metric_type in metric_types) {
			...
			if (length(perf_vs_time_data$full_obj_name) > 1 ) {
				...
				xy_lims <- plot_get_x_y_limits(perf_vs_time_data, xati, yati)
...
#				+ scale_y_continuous(limits = xy_lims[[3]], breaks = xy_lims[[4]])
				...
				if (en == "all") {
					...
					experiment_name <- paste('/', en, '/', sep = '')
					}
				file_location <- paste(ed, experiment_name, "vm_", metric_type, 
						"_vs_time_plot_", vapp_type, ".pdf", sep = '')
				
				ggsave(perf_vs_time_plot, file = file_location, width = sps)
				...
				if (length(perf_vs_load_data$full_obj_name) > 1 ) {
					...
					xy_lims <- plot_get_x_y_limits(perf_vs_load_data, xati, yati)	
...
#				+ scale_y_continuous(limits = xy_lims[[3]], breaks = xy_lims[[4]])
					...
					file_location <- paste(ed, experiment_name, "vm_", 
							metric_type, "_vs_load_plot_", vapp_type, ".pdf", 
							sep = '')

					ggsave(perf_vs_load_plot, file = file_location, width = sps)
...
plot_runtime_os_data <- function(rosmdf, ed, en, nnl, xati, yati, sps) {
	...
	for (experiment_node in experiment_node_list) {
		...
		os_cpu_plot_title <- msg

		xy_lims <- plot_get_x_y_limits(os_cpu_data, xati, yati)	

		os_cpu_plot <- ggplot(os_cpu_data, aes(x = relative_time, y = value, 
								colour = variable)) + 
				geom_point() + 
				xlab("Time (minutes)") + 
				ylab("CPU usage (%)") + 
				labs(title = os_cpu_plot_title) + 
				scale_x_continuous(limits = xy_lims[[1]], breaks = xy_lims[[2]]) 
#				+ scale_y_continuous(limits = xy_lims[[3]], breaks = xy_lims[[4]])
	
		experiment_name <- paste('/', en, '/', sep = '')
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_cpu_usage_vs_time_plot.pdf", sep = '')
		
		ggsave(os_cpu_plot, file = file_location, width = sps)
		...
		xy_lims <- plot_get_x_y_limits(os_mem_data, xati, yati)
	...
	#				+ scale_y_continuous(limits =  xy_lims[[3]], breaks =  xy_lims[[4]])
		...
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_mem_usage_vs_time_plot.pdf", sep = '')
		
		ggsave(os_mem_plot, file = file_location, width = sps)
		################## START Memory Usage vs Time ##################
		...
		xy_lims <- plot_get_x_y_limits(os_net_bw_data, xati, yati)
	...
	#				+ scale_y_continuous(limits =  xy_lims[[3]], breaks =  xy_lims[[4]])
		...
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_net_bw_vs_time_plot.pdf", sep = '')
		
		ggsave(os_net_bw_plot, file = file_location, width = sps)
		...
		xy_lims <- plot_get_x_y_limits(os_net_tput_data, xati, yati)
	...
	#				+ scale_y_continuous(limits =  xy_lims[[3]], breaks =  xy_lims[[4]])
		...
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_net_tput_vs_time_plot.pdf", sep = '')
		
		ggsave(os_net_tput_plot, file = file_location, width = sps)
		...
		xy_lims <- plot_get_x_y_limits(os_dsk_bw_data, xati, yati)
	...
	#				+ scale_y_continuous(limits =  xy_lims[[3]], breaks =  xy_lims[[4]])
		...
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_dsk_bw_vs_time_plot.pdf", sep = '')
		
		ggsave(os_dsk_bw_plot, file = file_location, width = sps)
		...
		xy_lims <- plot_get_x_y_limits(os_dsk_tput_data, xati, yati)
	...
	#				+ scale_y_continuous(limits =  xy_lims[[3]], breaks =  xy_lims[[4]])
		...
		file_location <- paste(ed, experiment_name, experiment_node, 
				"_dsk_tput_vs_time_plot.pdf", sep = '')
		
		ggsave(os_dsk_tput_plot, file = file_location, width = sps)
