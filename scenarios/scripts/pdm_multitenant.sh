...
then
    ...
    tbstr=$(tac)
    tbs=$?     
    TNSR_OUTPUT=$TNSR_OUTPUT",${tbstr}net_creation_time:${ndiff}"
    TNSR_OUTPUT=$TNSR_OUTPUT",netname:${PDM_NETWORK_NAME}-${counter}-1"
    TNSR_OUTPUT=$TNSR_OUTPUT",run_netname:${PDM_NETWORK_NAME}-${counter}-1"
    TNSR_OUTPUT=$TNSR_OUTPUT",prov_netname:${PDM_NETWORK_NAME}-${counter}-1"
