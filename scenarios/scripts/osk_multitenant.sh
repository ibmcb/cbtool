...
then
    ...
    then
        ...
        if [[ $A -eq 1 && $B -eq 1 ]]
        then
            idiff=$(echo "scale=2; ${idiff} - ${pdiff}" | bc)
        fi                                            
...
then
    ...
    tbstr=$(tac)
    tbs=$?      
    TNSR_OUTPUT=$TNSR_OUTPUT",tenant:${OSK_TENANT_NAME}-$counter,project:$TID,${tbstr}tenant_creation_time:${tdiff}"
    ...
    tbstr=$(tac)
    tbs=$?      
    TNSR_OUTPUT=$TNSR_OUTPUT",user:$OSK_USER_NAME-${counter},${tbstr}user_creation_time:$udiff"
    if [[ $TENANT_JUST_CREATED -eq 1 ]]
    then           
    ...
    tbstr=$(tac)
    tbs=$?          
    TNSR_OUTPUT=$TNSR_OUTPUT",${tbstr}net_creation_time:${ndiff},netname:${OSK_NETWORK_NAME}-${counter}-1,run_netname:${OSK_NETWORK_NAME}-${counter}-1,prov_netname:${OSK_NETWORK_NAME}-${counter}-1"        
