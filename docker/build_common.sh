...
CB_DROPBOX=${CB_DROPBOX:-"127.0.0.1"}
...
CB_FORCE_REBUILD=0
CB_USAGE="Usage: $0 [-r <repository>] [-u Ubuntu base image] [-c Centos base image] [-w Workload] [-l CB Username/login] [-b branch] [--verbose] [--push] [--psall] [-f]"
...
#       CB_ACTUAL_SQUASH="--squash"
    ...
    sudo sed -i "s^REPLACE_RSYNC_DOWNLOAD^rsync -a rsync://${_CB_RSYNC}/ --exclude 3rd_party/workload/ --exclude old_data/ --exclude tsam/ --exclude data/ --exclude jar/ --exclude windows/^g" Dockerfile
    ...
    then
        ...
        then
            ...
            if [[ $? -ne 0 ]]
            then
                echo "${_CB_REPOSITORY}$CB_DNAME:${_CB_BRANCH}" >> /tmp/cb_docker_failed
            fi
            sleep 5
        fi
    else
        echo "############## INFO: Image \"${_CB_REPOSITORY}$CB_DNAME\" built successfully!!!"
        sudo sed -i "s^${_CB_REPOSITORY}$CB_DNAME:${_CB_BRANCH}^^g" /tmp/cb_docker_failed > /dev/null 2>&1  
    fi
    sudo rm -rf Dockerfile
}
...
function cb_build_orchprereqs {
    ...
    do
        ...
        cb_docker_build $CB_REPOSITORY $CB_VERBQUIET $CB_DFILE latest $CB_USERNAME $CB_ARCH $CB_RSYNC true false $CB_DROPBOX
    done
...
function cb_build_orchestrator {
    ...
    for CB_DFILE in $(ls Dockerfile* | grep -v _processed_)
    do
        cb_docker_build $CB_REPOSITORY $CB_VERBQUIET $CB_DFILE $CB_BRANCH $CB_USERNAME $CB_ARCH $CB_RSYNC true false $CB_DROPBOX
    done
    echo "##### Done building Docker orchestrator images"
    echo
    popd > /dev/null 2>&1    
...
function cb_build_nullworkloads {
    ...
    do
        ...
        cb_docker_build $CB_REPOSITORY $CB_VERBQUIET $CB_DFILE $CB_BRANCH $CB_USERNAME $CB_ARCH $CB_RSYNC true true $CB_DROPBOX
    done
...
function cb_build_workloads {
    ...
    do
        cb_docker_build $CB_REPOSITORY $CB_VERBQUIET $CB_DFILE $CB_BRANCH $CB_USERNAME $CB_ARCH $CB_RSYNC false false $CB_DROPBOX
    done
