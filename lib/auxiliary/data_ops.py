...
def create_user_data_contents(obj_attr_list, osci) :
    ...
    # I don't understand what's the point of hacking out
    # the old VPN code if you don't actually start
    # and use the VPN before talking to redis...
    ...
    if obj_attr_list["use_vpn_ip"].lower() != "false" :
        ...
        # Can't start the VPN server before rsync finishes
        _userdata_contents += "while true ; do\n"
        _userdata_contents += "  if [ x\"$(ls  " + _rdfp + "/configs/generated/*" + obj_attr_list["cloud_name"] + "*client-cb-openvpn.conf 2>/dev/null)\" != x ] ; then\n"
    ...
    if obj_attr_list["use_vpn_ip"].lower() != "false" :
        ...
        _userdata_contents += "        redis-cli -h " + _hn + " -n " + str(osci.dbid) + " -p " + str(osci.port) + " hset TEST_" + _fsun + ':' + obj_attr_list["cloud_name"] + ":VM:PENDING:" + obj_attr_list["uuid"] + " cloud_init_vpn $VPNIP\n"
        _userdata_contents += "        break\n"    
        _userdata_contents += "    else\n"    
        _userdata_contents += "        sleep " + _sleep + "\n"
        _userdata_contents += "        counter=\"$(( $counter + 1 ))\"\n"    
