...
from libvirt import open as libvirt_open, libvirtError, VIR_DOMAIN_RUNNING
...
class PlmCmds(CommonCloudFunctions) :
    ...
    def __init__ (self, pid, osci, expid = None) :
        ...
        self.vhw_config["silver32"] = { "vcpus" : "4", "vmem" : "2048", "vstorage" : "35840", "vnics" : "1" }
        self.vhw_config["gold32"] = { "vcpus" : "4", "vmem" : "4096", "vstorage" : "35840", "vnics" : "1" }
        self.vhw_config["copper64"] = { "vcpus" : "2", "vmem" : "4096", "vstorage" : "61440", "vnics" : "1" }
        self.vhw_config["bronze64"]  = { "vcpus" : "2", "vmem" : "4096", "vstorage" : "87040", "vnics" : "1" }
        self.vhw_config["silver64"] = { "vcpus" : "4", "vmem" : "8192", "vstorage" : "104800", "vnics" : "1" }
        self.vhw_config["steel32"] = { "vcpus" : "8", "vmem" : "8192", "vstorage" : "104800", "vnics" : "1" }
        self.vhw_config["steel64"] = { "vcpus" : "6", "vmem" : "16384", "vstorage" : "104800", "vnics" : "1" }
        self.vhw_config["gold64"] = { "vcpus" : "8", "vmem" : "16384", "vstorage" : "104800", "vnics" : "1" }
    ...
    def connect(self, access, credentials, vmc_name, extra_parms = {}, diag = False, generate_rc = False) :
        ...
        try :
            ...
            for _endpoint in access.split(',') :
                if _endpoint.count(vmc_name) :
                    cbdebug("Registering: " + _endpoint)
                    _endpoint, _endpoint_name, _endpoint_ip= self.parse_endpoint(_endpoint, "qemu+tcp", False)

                    if vmc_name not in PlmCmds.lvirtconn :
                        PlmCmds.lvirtconn[vmc_name] = libvirt_open(_endpoint + ("/system" if not _endpoint.count("/system") else ""))
                        PlmCmds.lvirtconn[vmc_name].getSysinfo()
    ...
    def check_networks(self, vmc_name, vm_defaults) :
        ...
        for _vmc in list(PlmCmds.lvirtconn.keys()) :

            _msg = "Checking if the " + _net_str + " can be "
            _msg += "found on VMC " + vmc_name + " (endpoint " + _vmc + ")..."
            cbdebug(_msg, True)

            for _network in PlmCmds.lvirtconn[_vmc].listNetworks() :
        ...
        if not _prov_netname_found :
            ...
            _msg += " VMC " + vmc_name  + " (endpoint " + _vmc + ")..."
            _fmsg = _msg
            cberr(_msg, True)
    ...
    def check_images(self, vmc_name, vm_templates, poolname, vm_defaults) :
        ...
        for _vmc in list(PlmCmds.lvirtconn.keys()) :
            ...
            self.common_messages("IMG", { "name": vmc_name, "endpoint" : _vmc }, "checking", 0, '')

            _map_name_to_id = {}
            _map_id_to_name = {}

            _storage_pool_handle = PlmCmds.lvirtconn[_vmc].storagePoolLookupByName(poolname)
            _storage_pool_handle.refresh()
    ...
    def discover_hosts(self, obj_attr_list, start) :
        ...
        _vmc = obj_attr_list["name"]
        _host_info = PlmCmds.lvirtconn[_vmc].getInfo()
        _host_extended_info = PlmCmds.lvirtconn[_vmc].getSysinfo()
        ...
        obj_attr_list["host_list"][_host_uuid]["cloud_ip"] = hostname2ip(_vmc, True)
        ...
        obj_attr_list["host_list"][_host_uuid]["cloud_ip"] = _vmc
    ...
    def vmccleanup(self, obj_attr_list) :
        ...
        try :
            ...
            while _running_instances and _curr_tries < _max_tries :
                ...
                _domain_list = PlmCmds.lvirtconn[_vmc].listAllDomains()
            ...
            while _created_volumes and _curr_tries < _max_tries :
                ...
                for _storage_pool in _storage_pool_list :
                    _storage_pool_handle = PlmCmds.lvirtconn[_vmc].storagePoolLookupByName(_storage_pool)
    ...
    def get_ip_address(self, obj_attr_list) :
        ...
        # If connected to more than one hypervisor, let's support network overlays.
        # (For example VxLAN)
        # In such cases, there's only one DHCP server, but we don't know which
        # HV has it. So, let's just check them all.
        #
        # In the common case, we'll only have one HV anyway, so it's cool.

        for _vmc in PlmCmds.lvirtconn.keys() :
            _network_handle = PlmCmds.lvirtconn[_vmc].networkLookupByName(obj_attr_list["run_netname"])
            for _item in _network_handle.DHCPLeases() :
                if _item["mac"] == obj_attr_list["cloud_vm_mac"] :
                    ...
                    if str(obj_attr_list["ports_base"]).lower() != "false" :
                        obj_attr_list["prov_cloud_ip"] = _vmc_name
                    else :
    ...
    def get_instances(self, obj_attr_list, obj_type = "vm", endpoints = "all", identifier = "all") :
        ...
        if endpoints == "all" :
            _endpoints = list(PlmCmds.lvirtconn.keys())
        else :
        ...
        try :
            for _endpoint in _endpoints :
                if identifier == "all" :
                    ...
                    _instances = PlmCmds.lvirtconn[_endpoint].listAllDomains()

                else :
                    _call = "lookupByName()"
                    _instances = PlmCmds.lvirtconn[_endpoint].lookupByName(identifier)
    ...
    def get_images(self, obj_attr_list) :
        ...
        try :
            ...
            _storage_pool_handle = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].storagePoolLookupByName(obj_attr_list["poolname"])
    ...
    def get_networks(self, obj_attr_list) :
        ...
        try :
            ...
            _network_handle = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].networkLookupByName(obj_attr_list["netname"])
            ...
            if str(obj_attr_list["extra_netnames"]).lower() != "false" :
                for _exn in obj_attr_list["extra_netnames"].split(',') :
                    _network_handle = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].networkLookupByName(_exn)
    ...
    def is_vm_running(self, obj_attr_list):
        ...
        '''
        try :
            _host_ip = obj_attr_list["vmc_name"]
    ...
    def vmcreate(self, obj_attr_list) :
        ...
        try :
            ...
            _mark_a = time()
            _domain = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].defineXML(_xml_file)
            _domain.create()
    ...
    def vmcapture(self, obj_attr_list) :
        ...
        try :
            ...
            if _instance :
                ...
                _storage_pool_handle = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].storagePoolLookupByName(obj_attr_list["poolname"])
                _volume_handle = PlmCmds.lvirtconn[obj_attr_list["vmc_name"]].storageVolLookupByPath(_vol_path)
    ...
    def imgdelete(self, obj_attr_list) :
        ...
        try :
            ...
            for _vmc in PlmCmds.lvirtconn :
                ...
                _storage_pool_handle = PlmCmds.lvirtconn[_vmc].storagePoolLookupByName(obj_attr_list["poolname"])
    ...
    def configure_port_mapping(self, obj_attr_list, operation) :
        ...
        try :
            _proc_man = ProcessManagement(username = "root", \
                                          hostname = obj_attr_list["vmc_name"], \
                                          cloud_name = obj_attr_list["cloud_name"])
...
#        _xml_template += "\t<uuid>" + str(instance_attr_list["cloud_uuid"]) + "</uuid>\n"
        ...
        #if obj_attr_list["cloud_vm_name"].count("mysql") or obj_attr_list["cloud_vm_name"].count("redis") or obj_attr_list["cloud_vm_name"].count("linpack"):
        #    _xml_template += "\t<memoryBacking><hugepages/></memoryBacking>\n"
        #_xml_template += "\t<numatune><memory mode='strict' nodeset='1'/></numatune>\n"
...
#        _xml_template += "\t\t<pae/>\n"
        _xml_template += "\t</features>\n"
        _xml_template += "\t<cpu mode='host-passthrough'>\n"
...
#            _xml_template += "\t\t\t<mac address='" + str(obj_attr_list["cloud_vm_mac"]) + "'/>\n"
        ...
        if obj_attr_list["arch"]  == "ppc64" or obj_attr_list["arch"] == "ppc64le" :
            ...
            _xml_template += "\t\t<graphics type='vnc' port='-1' autoport='yes' listen='" + obj_attr_list["vmc_name"] + "' keymap='en-us'/>\n"
            _xml_template += "\t\t<video>\n"
    ...
    def ship_cloud_init_iso(self, obj_attr_list) :
        ...
        _geniso_cmd += " -input-charset utf-8 -volid cidata -joliet -rock user-data meta-data; "
        _geniso_cmd += "scp " +  _cloud_init_instance_path + '/' + _iso_fn
        _geniso_cmd += ' ' + obj_attr_list["host_remote_user"] + "@" + obj_attr_list["vmc_name"] + ":" + obj_attr_list["host_remote_dir"] + _iso_fn
